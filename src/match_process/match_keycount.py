...
def __match_type(field,matchd,data):
    ...
    if isinstance(keywords, list):
        ...
        if __starts_with_substring(keywords):
            filtered_data = [data_dict for data_dict in data if fj.filter(
...
def __filter_datas(input_dict: dict, new_data: list) -> list:
    ...
    def filter_data(number: int, data: list) -> list:
        try:
            ...
            if field and matchd:
                new_data = __match_type(field, matchd, data)
                return new_data
            else:
                return False
...
def cycle_statistics(input_data: list) -> list:
    try :
        ...
        for index, input_dict in enumerate(input_data):
            ...
            data_data = ExcelGeneralClass(file=data_file, sheet=input_dict['sheet']).read_data()
            data = __filter_datas(input_dict,data_data)
            count_list.append({'count_name': input_dict['count_name'], 'count_row': index+2, 'count_num': len(data), 'count_data': data})
        return count_list
